...
if command -v git >/dev/null 2>&1; then
  ...
  if [ -d "$REPO/core" ]; then
    ...
    cp "$REPO/launchers/universal.sh" "$TARGET_DIR/rl_universal.sh"
  else
    fetch "core/rl_bakkes_core.sh" "$TARGET_DIR/rl_bakkes_core.sh"
    fetch "launchers/universal.sh" "$TARGET_DIR/rl_universal.sh"
  fi
else
  fetch "core/rl_bakkes_core.sh" "$TARGET_DIR/rl_bakkes_core.sh"
  fetch "launchers/universal.sh" "$TARGET_DIR/rl_universal.sh"
...
chmod +x "$TARGET_DIR/rl_bakkes_core.sh" "$TARGET_DIR/rl_universal.sh"

say "Launching platform script: universal"
bash "$TARGET_DIR/rl_universal.sh" "$@"
