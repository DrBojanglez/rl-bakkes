...
if command -v git >/dev/null 2>&1; then
  ...
  if [ -d "$REPO/core" ]; then
    ...
    cp "$REPO/launchers/steamdeck.sh" "$TARGET_DIR/rl_steamdeck.sh"
  else
...
chmod +x "$TARGET_DIR/rl_bakkes_core.sh" "$TARGET_DIR/rl_steamdeck.sh"

say "Launching platform script: steamdeck"
bash "$TARGET_DIR/rl_steamdeck.sh" "$@"
