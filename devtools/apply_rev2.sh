...
REPO="$HOME/rl-bakkes"
REMOTE_HTTPS="https://github.com/DrBojanglez/rl-bakkes.git"

say(){ printf "[%s] %s\n" "$(date '+%H:%M:%S')" "$*"; }

# --- sanity ---
[ -d "$REPO/.git" ] || { say "Repo not found at $REPO"; exit 1; }
cd "$REPO"

# --- write NEW core (rev2) ---
cat > core/rl_bakkes_core.sh <<'EOF_CORE'
#!/usr/bin/env bash
...
EOF_CORE
chmod +x core/rl_bakkes_core.sh

# --- write NEW launchers (layout-resilient) ---
write_launcher() {
  local path="$1" paths="$2" wait="$3"
  cat > "$path" <<EOF_L
#!/usr/bin/env bash
set -euo pipefail
export PRIORITY_STEAM_PATHS="$paths"
export WAIT_SECS=$wait
...
write_launcher launchers/debian.sh    '$HOME/.steam/steam:$HOME/.local/share/Steam' 100
write_launcher launchers/steamdeck.sh '$HOME/.local/share/Steam:$HOME/.steam/steam' 120
write_launcher launchers/universal.sh '$HOME/.steam/steam:$HOME/.local/share/Steam' 100

# --- set remote to HTTPS and push ---
git remote | grep -qx origin || git remote add origin "$REMOTE_HTTPS" || true
git remote set-url origin "$REMOTE_HTTPS"
git add core/rl_bakkes_core.sh launchers/*.sh
git commit -m "rev2: handle missing Rocket League (prompt to install); launchers source core from same dir or repo layout" || true
git push -u origin main

say "Pushed rev2 to $REMOTE_HTTPS"
